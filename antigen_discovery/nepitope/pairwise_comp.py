...
class PairwiseComp(object):
    ...
    def __init__(self, list_dfs, min_nmer, original_fasta, threshold=None):
        ...
        self.original_proteins_df = pandas.DataFrame(self.peps_prots_ranges, columns=['ID', 'ProtSeq', 'Ranges'])
        self.original_proteins = self.original_proteins_df.ID.values.tolist()
    ...
    def pipe_run(self):
        ...
        for ref_df in self.filt_dfs:
            ...
            """
            list_dict_scores.append([self.run_pairwise_comp(ref_df), ref_df.ID.unique()])
    ...
    def write_csv_filtered_dfs(self, path):
        ...
        for i in self.filt_dfs:
            i.to_csv(path + i.ID.unique()[0] + '.csv', sep=',')
